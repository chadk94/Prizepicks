...
def get_urls(leagueurl):  # break this up ito multiple functions
    output = []
    headers = ['Day', 'Comp', 'Round', 'Venue', 'Result', 'Squad', 'Opponent', 'Start', 'Pos', 'Min', 'Gls', 'Ast', 'PK', 'PKatt', 'Shots', 'SoT', 'Yellows', 'Reds', 'Touches',
               'Press', 'Tackles', 'Interceptions', 'Blocks', 'xG', 'npxG', 'xA', 'SCA', 'GCA', 'Cmp', 'Att', 'Cmp%', 'ProgPass', 'Carries', 'Prog', 'Succ', 'Att', 'Report', 'Name']
    with open(op.join('../data', 'MLSdata.csv'), 'a', newline="", encoding="UTF-8") as f:
        write = csv.writer(f)
        ...
        for i in metric_names:
            ...
            for row in soup.findAll('table')[0].tbody.findAll('tr'):
                ...
                link = ("https://fbref.com/en/players/"+id +
                        "/matchlogs/2022/summary/"+name+"-Match-Logs")
                print(link)
