...
def call_endpoint(url, max_level=3, include_new_player_attributes=False):
    ...
    if include_new_player_attributes:
        inc_cop = included[included['type'] ==
                           'new_player'].copy().dropna(axis=1)
        data = pd.merge(data, inc_cop, how='left', left_on=[
                        'relationships.new_player.data.id', 'relationships.new_player.data.type'], right_on=['id', 'type'], suffixes=('', '_new_player'))
    return data


def getLines(league, bettype):
    # get outstanding lines in category for league and type
    ...
    df = df[['attributes.line_score', 'attributes.stat_type', 'attributes.name',
             'attributes.team', 'attributes.league', 'attributes.description']]
    df = df.dropna()
    return df
