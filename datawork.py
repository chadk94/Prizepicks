...
def clean(data,player):
    ...
    cleaned=data.drop(columns=['Result','Report','Comp','Round','Pos'])#todo reimplementPOs
    cleaned=cleaned.drop(columns=['Min','Gls','Ast','PK','PKatt', 'SoT', 'Yellows', 'Reds',
       'Touches', 'Press', 'Tackles', 'Interceptions', 'Blocks', 'xG', 'npxG',
       'xA', 'SCA', 'GCA', 'Cmp', 'Att', 'Cmp%', 'ProgPass', 'Carries', 'Prog',
       'Succ', 'Att.1'])
    print (cleaned.columns)
    cleaned.loc[len(cleaned.index)]=player
    cleaned=pd.get_dummies(cleaned, columns=['Day','Venue','Squad','Opponent','Start'])
